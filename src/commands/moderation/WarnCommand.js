...
module.exports = class WarnCommand extends BaseCommand {
  ...
  async run(client, message, args) {
    ...
    if (!message.guild.me.hasPermission("MANAGE_ROLES")) return message.channel.send('I require \`MANAGE ROLES\` permission to manage a users warnings.');
    ...
    if (!args[0]) return message.channel.send('you need to state a member along with if you are just cheching, adding or removing warnings.');
    ...
    if (!['add', 'remove'].includes(args[1])) {
      ...
      if (args[1] == 'add') {
        if (punishment == 1) {
          ...
          await mentionedMember.roles.add(warnRole3.id).catch(err => console.log(err));
          await mentionedMember.roles.remove(warnRole2.id).catch(err => console.log(err));
          return message.channel.send(`${mentionedMember}, you have warned for: ${reason} `);
        }
        ...
        if (punishment == 1) {
          ...
          await mentionedMember.roles.remove(warnRole1.id).catch(err => console.log(err));
          return message.channel.send(`${message.member}, I removed a warning from ${mentionedMember}`);
        } else if (punishment == 3) {
          await mentionedMember.roles.remove(warnRole2.id).catch(err => console.log(err));
          return message.channel.send(`${message.member}, I removed a warning from ${mentionedMember}`);
        } else if (punishment == 4) {
